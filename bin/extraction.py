...
def clean_gene_identifier(name):
    ...
    """
    final_name = name.replace("'", "").replace("-", "_").replace("(", "").replace(")", "").replace("/", "-") \
...
def make_extract_df_contig_col(extract_df):
    ...
    """
    extract_df = extract_df['Contig'].apply(lambda contig_name: str(contig_name).split('_')[0])
...
def get_annotation_gene_instances(extract_dataframes, col_tokens=None):
    ...
    for genome, df in extract_dataframes.items():
        ...
        start_end_coordinates = df['Gene_Start'].astype(str) + ':' + df['Gene_End'].astype(str)
...
def make_gene_neighborhood_df(GBK_df_dict, genome_id, gene_start, neighborhood_size,
                              modified_gene_name, annotation_instances=None):
    """
    ...
    contig_end_position = next((i for i, index in enumerate(downstream_indices) if index < 0), None)

    if contig_end_position is not None:
        print("Contig end found at position {} downstream.".format(contig_end_position + 1))
    ...
    for i in range(contig_end_position - 1, -1, -1):
        try:
            ...
            start_end_indices = '{}:{}'.format(neighbor['Gene_Start'].values[0], neighbor['Gene_End'].values[0])
            if start_end_indices in annotation_instances[genome_id.split('-')[0]].keys():
                if not neighbor['Gene_Name'].tolist()[0].startswith('UID'):
                    feature_name = annotation_instances[genome_id.split('-')[0]][
                                       start_end_indices] + f" ({neighbor['Gene_Name'].tolist()[0]})"
                else:
                    feature_name = annotation_instances[genome_id.split('-')[0]][start_end_indices]
                neighbor.loc[:, 'Gene_Name'] = feature_name
            downstream_neighbors = pd.concat([downstream_neighbors, neighbor])
        except IndexError:
            print("Contig end found at position {} downstream.".format(-(neighborhood_size - i)))
...
def get_all_gene_neighborhoods(gene_instance_dict, GBK_df_dict, neighborhood_size, cols=None, annotation_instances=None):
    """
    ...
    for gene, gene_dict in gene_instance_dict.items():
        ...
        for genome, data in gene_dict.items():
            ...
            for gene_instance in range(len(start_vals_list)):
                ...
                try:
                    genbank_gene_data = GBK_df.loc[((GBK_df['Gene_Start'] == start_index - 1))]
                    genbank_gene_name = genbank_gene_data.Gene_Name.tolist()[0].strip('"').strip("'")
                except IndexError:
                    try:
                        genbank_gene_data = GBK_df.loc[((GBK_df['Gene_Start'] == start_index))]
                        genbank_gene_name = genbank_gene_data.Gene_Name.tolist()[0].strip('"').strip("'")
                    except IndexError:
                ...
                if cols is not None:
                    ...
                    for col in cols:
                        additional_data.append(gene_dict[genome].loc[gene_instance, col])
                    delim = '_'
                    ...
                    if genbank_gene_name.startswith("UID"):
                        ...
                        modified_gene_name = gene + '_' + str_data + ' (' + genbank_gene_name + ')'
                else:
                    if genbank_gene_name.startswith("UID"):
                ...
                try:
                    neighborhood_df, indices = make_gene_neighborhood_df(GBK_df_dict, genome, start_index,
...
def extract_neighborhoods(input_file_path, extract_path, gbk_path, output_path,
    ...
    if type(input_file_data) is dict:
        ...
        annotation_instances = get_annotation_gene_instances(extract_dataframes, col_tokens)
        ...
        neighborhoods, neighborhood_indices = get_all_gene_neighborhoods(genomes_occurrence_dict_filtered,
                                                                         gbk_dataframes, num_neighbors, col_tokens,
                                                                         annotation_instances)
        ...
        for gene, neighborhood_data in neighborhoods.items():
            ...
            neighborhood_JSON_dict = make_neighborhood_JSON_data(neighborhood_data, neighborhood_indices, gene,
                                                                 num_neighbors)
    ...
    # Case 2: User provided list of genes to extract
        ...
        # 6) Extract gene neighborhoods and store them in dataframes
        neighborhoods, neighborhood_indices = get_all_gene_neighborhoods(genomes_occurrence_dict_filtered,
                                                                         gbk_dataframes, num_neighbors, label_cols)
        ...
        for gene, neighborhood_data in neighborhoods.items():
            # Get data needed to write JSON files
            neighborhood_JSON_dict = make_neighborhood_JSON_data(neighborhood_data, neighborhood_indices, gene, num_neighbors)
            # Create JSON file
            write_neighborhood_JSON(neighborhood_JSON_dict, gene, output_path)
